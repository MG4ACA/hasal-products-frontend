...
const breadcrumbItems = ref([{ label: 'Expenses', to: '/expenses' }, { label: 'Edit' }]);
...
const handleSubmit = async () => {
  ...
  try {
    ...
    await expenseStore.updateExpense(route.params.id, payload);
...
const loadExpense = async () => {
  loading.value = true;
  try {
    const expense = await expenseStore.fetchExpenseById(route.params.id);
    ...
    formData.value = {
      ...
      distance_km: expense.distance_km ? parseFloat(expense.distance_km) : null,
    };
  } catch (error) {
